...
def to_single_playing_direction(home,away,events):
    ...
    for team in [home,away,events]:
        second_half_start = team.loc[team['Period'] == 2].index.min()  # Option 1 or 2
        columns = [c for c in team.columns if c[-1].lower() in ['x','y']]
        team.loc[second_half_start:,columns] *= -1
...
def find_goalkeeper(team):
    '''
    Find the goalkeeper in a team, identifying him/her as the player closest to the goal at kick-off.
    ''' 
    # Filter columns that end in '_x' and start with 'Home' or 'Away'
    x_columns = [c for c in team.columns if c[-2:].lower() == '_x' and c[:4] in ['Home', 'Away']]
    
    # Find the column name of the goalkeeper by identifying the max value in the filtered columns
    GK_col = team.iloc[0][x_columns].abs().idxmax()
    
    # Split the column name to extract the player's number or identifier
